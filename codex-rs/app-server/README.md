...
`server/capabilities/read` reports fork-specific contracts without relying on the package version. The current response advertises Code Mode output reducer protocol version `2`, its two supported config keys, the `is_code_mode_nested` PostToolUse field, whether those reducer settings can be refreshed through `thread/resume`, and the v2 acceptance callback fields. A v2 descriptor supplies `acceptance_url`; a successful reduction response supplies a stable `response_id`; after Codex validates and commits to the replacement, it POSTs callback version `2` with that ID and the thread, turn, call, and cell IDs. The callback is a one-way commit notification: once Codex attempts it, a timeout or error response does not revoke the replacement, because the host may already have received the request. The host must finalize savings only when it receives a valid callback, never when the earlier reduction response finishes. A lost callback can therefore undercount savings but cannot create a false delivery claim. Clients should treat method-not-found as no reducer support.
