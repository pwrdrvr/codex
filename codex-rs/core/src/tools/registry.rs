...
impl ToolRegistry {
    ...
    pub(crate) async fn dispatch_any_with_terminal_outcome(
        ...
        {
            let code_mode_result = result.result.code_mode_result(&result.payload);
            invocation
                .session
                .services
                .code_mode_service
            ...
            Ok(mut result) => {
                if let Some(outcome) = post_tool_use_outcome {
                    ...
                    if let Some(feedback_message) = outcome.feedback_message {
                        ...
                        let managed_response_id = outcome.managed_response_id;
                        if !should_block {
                        ...
                        if !is_code_mode_nested {
                            ...
                            invocation
                                ...
                                .await;
                            if let Some(managed_response_id) = managed_response_id.as_deref() {
                                invocation
                                    .session
                                    .services
                                    .code_mode_service
